...
public class ThinDownloadManager implements DownloadManager {
	...
	/**
	 * Download request queue takes care of handling the request based on
	 * priority.
	 */
	...
	/**
	 * Constructor taking MAX THREAD POOL SIZE Allows maximum of 4 threads. Any
	 * number higher than four or less than one wont be respected.
	 **/
	...
	public ThinDownloadManager(String string, int type) {
		mRequestQueue = new DownloadRequestQueue();
		mRequestQueue.start2();
	}
	 ...
	 * number higher than four or less than one wont be respected.
	 **/
	public ThinDownloadManager(int threadPoolSize, int type) {
		mRequestQueue = new DownloadRequestQueue(threadPoolSize);
		mRequestQueue.start2();
	}

	/**
	 * Add a new download. The download will start automatically once the
	 * download manager is ready to execute it and connectivity is available.
	 * 
	 * @param request
	 *            the parameters specifying this download
	 * @return an ID for the download, unique across the application. This ID is
