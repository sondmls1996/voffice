...
import com.viettel.voffice.utils.Utils;
...
public class DownloadRequestQueue {
	...
	/**
	 * Delivery class to delivery the call back to call back registrar in main
	 * thread.
	 */
	class CallBackDelivery {
		...
		public void postDownloadFailed(final DownloadRequest request,
		...
		public void postProgressUpdate(final DownloadRequest request,
				final long totalBytes, final long downloadedBytes,
				final int progress) {
			mCallBackExecutor.execute(new Runnable() {
				public void run() {
					request.getDownloadListener().onProgress(
							request.getDownloadId(), totalBytes,
							downloadedBytes, progress);
				}
			});
		}

		public void postConnectHeader(final DownloadRequest request) {
			mCallBackExecutor.execute(new Runnable() {
				public void run() {
					request.getDownloadListener().onConnectHeader(
							Utils.getTimeDevice());
				}
			});
		}
	...
	/**
	 * Generates a download id for the request and adds the download request to
	 * the download request queue for the dispatchers pool to act on
	 * immediately.
	 * 
	 * @param request
	 * @return downloadId
	...
	/**
	 * Cancel a particular download in progress. Returns 1 if the download Id is
	...
	void finish(DownloadRequest request) {
		if (mCurrentRequests != null) {// if finish and release are called
										// together it throws NPE
	...
	/**
	 * Cancels all the pending & running requests and releases all the
	 * dispatchers.
	 */
	void release() {
