...
/**
 * {@code PullToRefreshXmlConfiguration} is an information set of PullToRefresh.
 * It contains a list of indicator layouts and of loading layouts. An
 * information set of PullToRefresh is converted from pulltorefresh.xml.<br />
 * This Class is Singleton, and you <b>MUST</b> call {@link #init(Context)}
...
public class PullToRefreshXmlConfiguration {
	 ...
	 * Load 'res/xml/pulltorefresh.xml' in PullToRefresh library package and
	 * 'assets/pulltofresh.xml' in Android Project if it exists. <br />
	 * Combine information of 'res/xml/pulltorefresh.xml' and
	 * 'assets/pulltofresh.xml', and then save the information into the
	 * instance's fields.<br />
	 * <br />
	 * NOTE: This method <b>MUST</b> be called before using!
	 * 
	 * @param context
	 *            Context instance and not null
	 */
	public void init(Context context) {
		...
		try {
			...
			// load extended xml
			XmlPullParser extendedXmlParser = ExtendedConfigXmlParserFactory
					.createParser(context);
			if (extendedXmlParser != null) {
				XmlPullParserWrapper extendedXmlWrapper = new XmlPullParserWrapper(
						extendedXmlParser);
				// NOTE : if some exception is thrown from
				// PullToRefreshConfigXmlParser, Loading extended xml will be
				// skipped.
				PullToRefreshNode extendedNode = new PullToRefreshConfigXmlParser(
						extendedXmlWrapper).parse();
	 ...
	 *            Layout name
	 * @return Layout Class name ( ex:
	 *         com.handmark.pulltorefresh.library.internal.DefaultLoadingLayout
	 *         )
	 */
	 ...
	 * @return Layout Class name ( ex:
	 *         com.handmark.pulltorefresh.library.internal
	 *         .DefaultGoogleStyleProgressLayout )
	 */
	...
	/**
	 * @return throw an exception if {@link #init(Context)} method has not been
	 *         called
	 */
	private void assertInitialized() {
		if (notInitialized()) {
			throw new IllegalStateException(
					PullToRefreshXmlConfiguration.class.getName()
							+ " has not initialized. Call init() method first.");
		}
	}
}
