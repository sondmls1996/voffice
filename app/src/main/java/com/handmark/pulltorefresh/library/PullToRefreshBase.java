...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	 ...
	 * (Don't set to false as possible, it's hard to control height if this flag
	 * is false)
	 */
	@SuppressWarnings("unused")
			...
			if (isReadyForPull()) {
				...
				if (absDiff > mTouchSlop
						&& (!mFilterTouchEvents || absDiff > Math
								.abs(oppositeDiff))) {
					if ((mMode.showHeaderLoadingLayout() || mMode
							.showGoogleStyle())
							&& diff >= 1f
							&& isReadyForPullStart()) {
						mLastMotionY = y;
	 ...
	 * 
	 * @param friction
	 *            New friction value. Must be float. The default value is
	 ...
	 * 
	 * @param smoothScrollDurationMs
	 *            Milliseconds. The default value is
	 *            {@value #DEFAULT_SMOOTH_SCROLL_DURATION_MS}.
	 */
	...
	/**
	 * Set new smooth scroll <b>longer</b> duration. <br />
	 * This duration is only used by calling {@link #smoothScrollToLonger(int)}.
	 * 
	 * @param smoothScrollLongDurationMs
	 *            Milliseconds. The default value is
	 *            {@value #DEFAULT_SMOOTH_SCROLL_LONG_DURATION_MS}.
	 */
	 ...
	 * Other methods Use this method instead of
	 * {@link #getPullToRefreshScrollDirection()} method, because an orientation
	 * must be VERTICAL when mode is google style
	 * </p>
	 * 
	 * @return Oreintation.VERTICAL if mMode.showGoogleStyle() is true,<br />
	 *         Return value of {@link #getPullToRefreshScrollDirection()} method
	 *         if else
	 */
	...
	/**
	 * Create a new loading layout instance by using the class token
	 * {@link #mLoadingLayoutClazz}
	 * 
	 * @param context
	 * @param mode
	 * @param attrs
	 * @return Loading layout instance which was created by using the class
	 *         token {@link #mLoadingLayoutClazz}
	 */
	protected LoadingLayout createLoadingLayout(Context context, Mode mode,
			TypedArray attrs) {
		return LoadingLayoutFactory
				.createLoadingLayout(mLoadingLayoutClazz, context, mode,
						getFilteredPullToRefreshScrollDirection(), attrs);
	}

	/**
	 * Create a new google style progress layout instance by using the class
	 * token
	 * 
	 * @param layoutCode
	 *            google style progress layout code to be converted to some
	 *            class token
	 * @param context
	 * @param mode
	 * @param attrs
	 * @return Google style <b>progress</b> layout instance which was created by
	 *         using the class token
	 */
	/**
	...
	protected void onRefreshing(final boolean doScroll) {
		// Set the flag as below for fade-in animation of mRefreshableView when
		// releasing
		mRefreshing = true;
		...
		if (mMode.showGoogleStyle()) {
			...
			if (mRefeshableViewHideWhileRefreshingEnabled == true) {
				AlphaAnimator.fadeout(mRefreshableView,
						mRefeshableViewHideWhileRefreshingDuration);
			}
			...
			if (mRefeshableViewRefreshingBarViewWhileRefreshingEnabled == true) {
				...
				AlphaAnimator
						.fadein(mRefreshableViewProgressBar,
								mRefeshableViewRefreshingBarViewWhileRefreshingDuration);
			}
	...
	protected final void refreshLoadingViewsSize() {
		...
		case VERTICAL:
			if (mMode.showHeaderLoadingLayout()) {
				...
				/**
				 * Set size of {@code GoogleStyleViewLayout} to ActionBar's size
				 * if {@code mSetGoogleViewLayoutSizeToActionbarHeight} is true
				 * This code is a default action, but if you want to use custom
				 * size of {@code GoogleStyleViewLayout}, set
				 * {@code ptrSetGoogleViewLayoutSizeToActionbarHeight} to false
				 * in layout xml (but not recommended).
				 */
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			...
			ViewCompat.setLayerType(mRefreshableViewWrapper,
					value != 0 ? LAYER_TYPE_HARDWARE : LAYER_TYPE_NONE /*
																		 * View.
																		 * LAYER_TYPE_NONE
																		 */);
		}
	...
	protected void updateUIForGoogleStyleMode() {
		...
		//
		/**
		 * Set size of {@code GoogleStyleViewLayout} to ActionBar's size if
		 * {@code mSetGoogleViewLayoutSizeToActionbarHeight} is true This code
		 * is a default action, but if you want to use custom size of
		 * {@code GoogleStyleViewLayout}, set
		 * {@code ptrSetGoogleViewLayoutSizeToActionbarHeight} to false in
		 * layout xml (but not recommended).
		 */
	...
	private void callRefreshListener() {
		if (null != mOnRefreshListener) {
			...
			if (mCurrentMode == Mode.PULL_FROM_START
					|| mCurrentMode == Mode.GOOGLE_STYLE) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Getting mMode is first, because It uses mMode in
		// getFilteredPullToRefreshScrollDirection()
		...
		// Default value of PTR View's gravity is center. So let the value be
		// set center when the gravity is not set yet in XML.
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrAnimationStyle)) {
			loadingLayoutCode = a
					.getString(R.styleable.PullToRefresh_ptrAnimationStyle);
		}

		mLoadingLayoutClazz = LoadingLayoutFactory
				.createLoadingLayoutClazzByLayoutCode(loadingLayoutCode);
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrGoogleViewStyle)) {
			googleStyleViewLayoutCode = a
					.getString(R.styleable.PullToRefresh_ptrGoogleViewStyle);
		}
		// Get a Google style progress layout class token
		String googleStyleProgressLayoutCode = null;
		if (a.hasValue(R.styleable.PullToRefresh_ptrGoogleViewStyle)) {
			googleStyleProgressLayoutCode = a
					.getString(R.styleable.PullToRefresh_ptrGoogleProgressStyle);
		}
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrShowGoogleStyleViewAnimationEnabled)) {
			mShowGoogleStyleViewAnimationEnabled = a
					.getBoolean(
							R.styleable.PullToRefresh_ptrShowGoogleStyleViewAnimationEnabled,
							true);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrHideRefeshableViewWhileRefreshingEnabled)) {
			mRefeshableViewHideWhileRefreshingEnabled = a
					.getBoolean(
							R.styleable.PullToRefresh_ptrHideRefeshableViewWhileRefreshingEnabled,
							true);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrViewRefeshableViewProgressBarOnCenterWhileRefreshingEnabled)) {
			mRefeshableViewRefreshingBarViewWhileRefreshingEnabled = a
					.getBoolean(
							R.styleable.PullToRefresh_ptrViewRefeshableViewProgressBarOnCenterWhileRefreshingEnabled,
							true);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrShowGoogleStyleViewAnimationDuration)) {
			mShowGoogleStyleViewAnimationDuration = a
					.getInteger(
							R.styleable.PullToRefresh_ptrShowGoogleStyleViewAnimationDuration,
							GOOGLE_STYLE_VIEW_APPEAREANCE_DURATION);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrHideRefeshableViewWhileRefreshingDuration)) {
			mRefeshableViewHideWhileRefreshingDuration = a
					.getInteger(
							R.styleable.PullToRefresh_ptrHideRefeshableViewWhileRefreshingDuration,
							REFRESHABLE_VIEW_HIDE_WHILE_REFRESHING_DURATION);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrViewRefeshableViewProgressBarOnCenterWhileRefreshingDuration)) {
			mRefeshableViewRefreshingBarViewWhileRefreshingDuration = a
					.getInteger(
							R.styleable.PullToRefresh_ptrViewRefeshableViewProgressBarOnCenterWhileRefreshingDuration,
							REFRESHABLEVIEW_REFRESHING_BAR_VIEW_WHILE_REFRESHING_DURATION);
		}

		// Get a flag that decides Google View Layout's size is set to
		// ActionBar's
		if (a.hasValue(R.styleable.PullToRefresh_ptrSetGoogleViewLayoutSizeToActionbarHeight)) {
			mSetGoogleViewLayoutSizeToActionbarHeight = a
					.getBoolean(
							R.styleable.PullToRefresh_ptrSetGoogleViewLayoutSizeToActionbarHeight,
							true);
		}
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrRefeshableViewProgressBarOnCenterWidth)) {
			mRefeshableViewRefreshingBarWidth = a
					.getInteger(
							R.styleable.PullToRefresh_ptrRefeshableViewProgressBarOnCenterWidth,
							DFEAULT_REFRESHABLEVIEW_REFRESHING_BAR_SIZE);
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrRefeshableViewProgressBarOnCenterHeight)) {
			mRefeshableViewRefreshingBarHeight = a
					.getInteger(
							R.styleable.PullToRefresh_ptrRefeshableViewProgressBarOnCenterHeight,
							DFEAULT_REFRESHABLEVIEW_REFRESHING_BAR_SIZE);
		}
		/**
		 * Styleables from XML
		...
		float friction = a.getFloat(R.styleable.PullToRefresh_ptrFriction,
				DEFAULT_FRICTION);
		int smoothScrollDuration = a.getInt(
				R.styleable.PullToRefresh_ptrSmoothScrollDuration,
				DEFAULT_SMOOTH_SCROLL_DURATION_MS);
		int smoothScrollLongDuration = a.getInt(
				R.styleable.PullToRefresh_ptrSmoothScrollLongDuration,
				DEFAULT_SMOOTH_SCROLL_LONG_DURATION_MS);
		...
		updateUIForMode();
		// updateUIForGoogleStyleMode() method will be called when
		// onAttachedToWindow() event has been fired.
	}
	...
	/**
	 * NOTE : This method must be called after initStatusBarSize() and
	 * initActionBarSize() have already been called. Also,
	 * mGoogleStyleProgressLayout should be initialized before calling this
	 * method.
	 */
	@SuppressWarnings("unused")
	...
	private void showViewTopLayout() {
		...
		if (mShowGoogleStyleViewAnimationEnabled == true) {
			...
			TranslateAnimation transAnimation = new TranslateAnimation(
					...
					Animation.ABSOLUTE, -mActionBarHeight, Animation.ABSOLUTE,
	...
	private void hideViewTopLayout() {
		...
		if (mShowGoogleStyleViewAnimationEnabled == true) {
			...
			TranslateAnimation transAnimation = new TranslateAnimation(
					Animation.ABSOLUTE, 0, Animation.ABSOLUTE, 0,
					Animation.ABSOLUTE, mTopActionbarLayout.getTop(),
					Animation.ABSOLUTE, -mStatusBarHeight);
	...
	/**
	 * Initialize {@code mTopActionbarLayout} and add that into Top
	 * DecorView(this is the root view), and initialize needed components
	 */
	private void initTopViewGroup() {
		...
		ViewGroup.LayoutParams params = new ViewGroup.LayoutParams(matchParent,
				mActionBarHeight);
		...
		if (mMode.showGoogleStyle()) {
			...
			FrameLayout.LayoutParams barParams = new FrameLayout.LayoutParams(
					mRefeshableViewRefreshingBarWidth,
			...
			mRefreshableViewWrapper.addView(mRefreshableViewProgressBar, -1,
					barParams);
		}
	...
	private final void smoothScrollToAndBack(int y) {
		smoothScrollTo(y, mSmoothScrollDurationMs, 0,
				new OnSmoothScrollFinishedListener() {

					@Override
					public void onSmoothScrollFinished() {
						smoothScrollTo(0, mSmoothScrollDurationMs,
	...
	public static enum Mode {
		...
		/**
		 * @return true if this mode wants the Loading Layout to be shown like
		 *         Google style pull-to-refresh
		 */
