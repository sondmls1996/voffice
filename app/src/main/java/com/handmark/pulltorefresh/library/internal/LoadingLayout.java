...
public abstract class LoadingLayout extends FrameLayout implements
	 ...
	 */
	protected LoadingLayout(Context context, final Mode mode,
			final Orientation scrollDirection, TypedArray attrs, int inflateId) {
		super(context);
		mMode = mode;
	...
	public LoadingLayout(Context context, final Mode mode,
		...
		if (null == mImageDrawable) {
			mImageDrawable = context.getResources().getDrawable(
	...
	/**
	 * Load labels of pull, refresh, release, and assign into fields <br />
	 * Convert an each attribute value such as {@code ptrPullLabel},
	 * {@code ptrRefreshLabel} or {@code ptrReleaseLabel} to each label field if
	 * each value exists. <br />
	 * Or if not, then the each label is assigned some string as default <br />
	 * NOTE : This method <b>Must</b> be modified if kinds of {@code Mode} are
	 * increased.
	 * 
	 * @param attrs
	 * @param mode
	...
	/**
	 * Load labels of pull <br />
	 * Convert an {@code ptrPullLabel} attribute value to {@code mPullLabel}
	 * field if each value exists. <br />
	 * Or if not, then the pull label is assigned some string as default <br />
	 * If you want to set some custom pull label at sub class, you have to
	 * override this method and implement. NOTE : This method <b>Must</b> be
	 * modified if kinds of {@code Mode} are increased.
	 * 
	 * @param context
	 * @param attrs
	 * @param mode
	...
	protected String loadPullLabel(Context context, TypedArray attrs, Mode mode) {
		...
		int stringId = (mode == Mode.PULL_FROM_END) ? R.string.pull_to_refresh_from_bottom_pull_label
				: R.string.pull_to_refresh_pull_label;
		return context.getString(stringId);
	}

	/**
	 * Load labels of refreshing <br />
	 * Convert an {@code ptrRefreshLabel} attribute value to
	 * {@code mRefreshingLabel} field if each value exists. <br />
	 * Or if not, then the refreshing label is assigned some string as default <br />
	 * If you want to set some custom refreshing label at sub class, you have to
	 * override this method and implement. NOTE : This method <b>Must</b> be
	 * modified if kinds of {@code Mode} are increased.
	 * 
	 * @param context
	...
	protected String loadRefreshingLabel(Context context, TypedArray attrs,
		...
		int stringId = (mode == Mode.PULL_FROM_END) ? R.string.pull_to_refresh_from_bottom_refreshing_label
				: R.string.pull_to_refresh_refreshing_label;
		return context.getString(stringId);
	}

	/**
	 * Load labels of release <br />
	 * Convert an {@code ptrReleaseLabel} attribute value to
	 * {@code mReleaseLabel} field if each value exists. <br />
	 * Or if not, then the release label is assigned some string as default <br />
	 * If you want to set some custom release label at sub class, you have to
	 * override this method and implement. NOTE : This method <b>Must</b> be
	 * modified if kinds of {@code Mode} are increased.
	 * 
	 * @param context
	...
	protected String loadReleaseLabel(Context context, TypedArray attrs,
		...
		int stringId = (mode == Mode.PULL_FROM_END) ? R.string.pull_to_refresh_from_bottom_release_label
	...
	private void hideHeaderProgress() {
		if (null != mHeaderProgress
				&& View.VISIBLE == mHeaderProgress.getVisibility()) {
	...
	private void hideSubHeaderText() {
		if (null != mHeaderText
				&& View.VISIBLE == mSubHeaderText.getVisibility()) {
	...
	private void showSubHeaderText() {
		if (null != mSubHeaderText
				&& View.INVISIBLE == mSubHeaderText.getVisibility()) {
			mSubHeaderText.setVisibility(View.VISIBLE);
		}
	}
	...
	private void showHeaderProgress() {
		if (null != mHeaderProgress
				&& View.INVISIBLE == mHeaderProgress.getVisibility()) {
	...
	private void showHeaderText() {
		if (null != mHeaderText
				&& View.INVISIBLE == mHeaderText.getVisibility()) {
