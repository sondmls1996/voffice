...
public class PullToRefreshListView extends
	...
	public FrameLayout getFrameFooter() {
		return mLvFooterLoadingFrame;
	}

	public void setmFrameFooter(FrameLayout mLvFooterLoadingFrame) {
		this.mLvFooterLoadingFrame = mLvFooterLoadingFrame;
	}

	public FrameLayout getFrameHeader() {
		return frameHeader;
	}

	public void setFrameHeader(FrameLayout frameHeader) {
	...
	protected void onRefreshing(final boolean doScroll) {
		...
		/**
		 * If we're not showing the Refreshing view, the the header/footer views
		 * won't show so we use the normal method.
		 */
		if (!mListViewExtrasEnabled || !getShowViewWhileRefreshing()
				|| getCurrentMode() == Mode.GOOGLE_STYLE) {
	...
	protected void handleStyledAttributes(TypedArray a) {
		...
		if (mListViewExtrasEnabled) {
			...
			frameHeader = new FrameLayout(getContext());
			mHeaderLoadingView = createLoadingLayout(getContext(),
					Mode.PULL_FROM_START, a);
			mHeaderLoadingView.setVisibility(View.GONE);
			frameHeader.addView(mHeaderLoadingView, lp);
			mRefreshableView.addHeaderView(frameHeader, null, false);
