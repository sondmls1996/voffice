...
/**
 * Factory which creates loading layouts <br />
 * Loading layouts must be listed in pulltorefresh.xml as
 * "PullToRefresh/LoadingLayouts/layout" nodes
 * 
 * @author Wonjun Kim
 */
class LoadingLayoutFactory {
	 ...
	 * 
	 * @param layoutCode
	 *            Loading layout code, which must be defined in
	 *            pulltorefresh.xml
	 * @return Class token which is matched by {@code layoutCode}, or the class
	 *         token of {@code RotateLoadingLayout} instance if not
	 */
	public static Class<? extends LoadingLayout> createLoadingLayoutClazzByLayoutCode(
			String layoutCode) {
		String clazzName = PullToRefreshXmlConfiguration.getInstance()
				.getLoadingLayoutClazzName(layoutCode);
	 ...
	 * @param context
	 * @param mode
	 * @return {@code LoadingLayout} instance if the class matched by
	 *         {@code layoutCode} exists, or {@code RotateLoadingLayout}
	 *         instance if not
	 */
	...
	public static Class<? extends LoadingLayout> createLoadingLayoutClazz(
		...
		if (clazzName == null) {
			loadingLayoutClazz = DefaultLoadingLayoutFactory
					.createLoadingLayoutClazz(clazzName);
			return loadingLayoutClazz;
		}

		try {
			loadingLayoutClazz = (Class<LoadingLayout>) Class
					.forName(clazzName);
	...
	/**
	 * Create a {@code LoadingLayout} instance matched by <b>{@code layoutCode}
	 * </b>
	 * 
	 * @param layoutCode
	 *            Loading layout code, which must be defined in
	 ...
	 *         instance if not
	 */
	public static LoadingLayout createLoadingLayout(String layoutCode,
			Context context, Mode mode, Orientation orientation,
			TypedArray attrs) {
	...
	public static LoadingLayout createLoadingLayout(
		...
		if (layout == null) {
			layout = DefaultLoadingLayoutFactory.createLoadingLayout(clazz,
					context, mode, orientation, attrs);
		}
	...
	/**
	 * Factory which creates a default loading layout instance. This is used
	 * when {@code LoadingLayoutFactory} fails to create a instance
	 * 
	 * @author Wonjun Kim
	 * 
	 */
