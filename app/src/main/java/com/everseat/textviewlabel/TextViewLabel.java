...
public class TextViewLabel extends TextView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray ta = context.obtainStyledAttributes(attrs,
					R.styleable.TextViewLabel);
			ColorStateList color = ta
					.getColorStateList(R.styleable.TextViewLabel_labelColor);
			if (color != null) {
		...
		if (getPaddingLeft() == 0 || getPaddingRight() == 0
				|| getPaddingBottom() == 0 || getPaddingTop() == 0) {
	...
	/**
	 * Sets the corner radius on the label. By default the corner radius is
	 * 1/20th of the labels width.
	 * 
	 * @param cornerRadius
	...
	private void drawLabel(Canvas canvas, float cornerRadius) {
		canvas.drawRoundRect(labelBounds, cornerRadius, cornerRadius,
				labelPaint);
	}
}